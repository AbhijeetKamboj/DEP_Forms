...
export default async function DeanFaaIdentityDashboardPage({
  searchParams,
}: {
  ...
  const guestHouseQueueHref = isQueueOpen
    ? "/dashboard/guest-house/approving-authority?queue=open&returnTo=/dashboard/identity-card/dean-faa?queue=open"
    : "/dashboard/guest-house/approving-authority?queue=open&returnTo=/dashboard/identity-card/dean-faa";

  await requireRole(["DEAN_FAA", "SYSTEM_ADMIN"]);
  ...
  const guestHouseStage1Count = (await listGuestHouseFormsForStage(1)).filter((form) =>
    canRoleApproveGuestHouseStage1("DEAN_FAA", {
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="mb-2 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-slate-900">Dean FA&A Dashboard</h1>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="relative">
              <Link
                href={
                  isQueueOpen
                    ? "/dashboard/identity-card/dean-faa"
                    : "/dashboard/identity-card/dean-faa?queue=open"
                }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-lg text-amber-700">
                Category-based competent authority requests assigned to Dean FA&A.
              </p>
            </div>
            <div className="relative">
              <Link
                href={guestHouseQueueHref}
                className="rounded-2xl bg-black px-8 py-4 text-base font-semibold text-white transition hover:bg-slate-800"
              >
