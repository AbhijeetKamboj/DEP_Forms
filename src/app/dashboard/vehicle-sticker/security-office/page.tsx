...
export default async function SecurityOfficeVehicleStickerDashboardPage({
  searchParams,
}: {
  ...
  const pendingForms = await listVehicleStickerFormsForStage(4);
  const awaitingFromStage1 = await listVehicleStickerFormsForStage(1);
  const awaitingFromStage2 = await listVehicleStickerFormsForStage(2);
  const awaitingFromStage3 = await listVehicleStickerFormsForStage(3);
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="mb-2 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-slate-900">Security Office Dashboard</h1>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-lg text-amber-700">Open queue to review pending Security Office requests.</p>
            </div>
            <div className="relative">
              <Link
                href={
                  isQueueOpen
                    ? "/dashboard/vehicle-sticker/security-office"
                    : "/dashboard/vehicle-sticker/security-office?queue=open"
                }
